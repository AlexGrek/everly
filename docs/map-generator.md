...
1. **Subseed rooms** — axis-aligned rectangles grow only from subseed centers (radius 8–15 cells → 17–31 tile footprints). Each rect must be at least **3×3** cells ([`MIN_ROOM_DIM`](../../src/map/map_generator/types.rs)).
2. **Houses** — only **overlapping** subseed rects merge into one [`House`](../../src/map/map_generator/house.rs) (edge-touching rects stay separate; subseed identity discarded). Each chunk targets **8–12** primary seeds (fewer when `MIN_SEED_DISTANCE` 18 cannot fit more) and **2–4** subseeds per primary. Seeds and footprints respect a **2-tile safe zone** (`BORDER_CLEARANCE`) in from the void margin on every side.
...
7. **Inner room walls** — [`step_split_houses_into_rooms`](../../src/map/map_generator/step_inner_walls.rs) runs on houses with `footprint_area >= 100` (`MIN_HOUSE_AREA_FOR_INNER_WALLS`). Each house rolls **1…min(floor(area / 80), 6)** inner wall lines (random), split ceiling-to-horizontal / floor-to-vertical, capped at **3 per axis**. A cut is kept only when every resulting sub-room is **≥ 3 cells in each dimension**, ≥ 9 cells in bbox area, and ≥ 3 cells away from every existing parallel wall (outer or inner). Stamps `MASK_NORTH` / `MASK_WEST` on cells along the line; concave voids and existing corner pillars are skipped, and **both outer door cells** (`wall_x/wall_z` and `wall2`) are never re-sealed. Rooms are isolated — no inner doors are placed at this step.
...
2. On commit, `generate_house_tiles(width, height, seed)` builds a padded `MapDraft`
   ...
   `MIN_HOUSE_TOOL_SIDE` (`18`) on either side return `None`.

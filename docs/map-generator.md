...
6. **One door per house (2-wide)** — [`step_place_house_doors`](../../src/map/map_generator/step_door.rs) picks a validated site per house: exterior tile must be road (not inside any house footprint), interior must be open floor, single-bit wall slab (no L-corner traps), and the doorway must not face another house’s wall. The step **prefers sites with a valid adjacent perimeter cell along the wall run** so both cells can be opened, yielding a **2-tile-wide doorway**. When no widenable site exists (degenerate geometry), a 1-wide door is placed as a fallback. The second door cell (if any) is stored in `HouseEntrypoint.wall2`.
...
10. **Charging stations** — [`step_place_charging_stations`](../../src/map/map_generator/step_charging_stations.rs) places **one** `Charger` per house: a walkable interior `Open` cell with **exactly one** orthogonal wall neighbor (so its back faces that wall and it is **not** in a corner), excluding **both** outer door cells and both of their inner tiles. The lone wall side becomes the [`ChargerFacing`](../../src/map/world_map.rs). Runs **last** (after crawlers) so the marble/glass waves are unaffected; chargers stay passable. A house with no qualifying cell simply gets none.

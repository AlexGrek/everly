    ...
    T: Clone + Send + Sync + 'static,
{
    fn new(fill: &T) -> Self {
        Self {
            cells: vec![fill.clone(); HYPERMAP_CHUNK_AREA],
        }
    }

    pub fn get_local(&self, local: LocalCoord) -> &T {
        &self.cells[local_to_index(local)]
    }

    pub fn set_local(&mut self, local: LocalCoord, value: T) {
        self.cells[local_to_index(local)] = value;
    }
}
    ...
    T: Clone + Send + Sync + 'static,
{
    ...
    /// Returns tile value at world coordinate. Missing chunks read as default tile.
    pub fn get(&self, world_x: i32, world_y: i32) -> T {
        ...
        if let Some(chunk_handle) = self.get_chunk(chunk) {
            ...
            guard.get_local(local).clone()
        } else {
            self.default_tile.clone()
        }
    }

    /// Writes tile value at world coordinate, creating chunk lazily if needed.
    pub fn set(&self, world_x: i32, world_y: i32, value: T) {
        ...
        guard.set_local(local, value);
    }

    /// Updates a world tile in place.
        ...
        F: FnOnce(&mut T),
    {
        ...
        let chunk_handle = self.get_or_create_chunk(chunk_coord);
        let mut guard = chunk_handle.write().expect("chunk lock poisoned");
        f(&mut guard.cells[local_to_index(local)]);
    }
